...
admin_keyboard = [
    ["📝 ثبت عضویت", "💳 پرداخت"],
    ["📋 لیست اعضا", "✅ تایید پرداخت"],
    ["🗑️ ریست کل اطلاعات", "ℹ️ راهنما"]
]
...
def show_reset_confirmation(update, context):
    ...
    confirmation_text = (
        ...
        "❌ **این عمل تمام اطلاعات را حذف می‌کند**\n\n"
...
def process_reset_confirmation(update, context):
    ...
    if text.strip() == "تایید ریست":
        if group_id in data:
            ...
            success_text = "✅ **ریست کامل اطلاعات انجام شد!**\n\n📝 اعضا می‌توانند از نو ثبت‌نام کنند."
        ...
        update.message.reply_text("❌ ریست لغو شد.")
        context.user_data['waiting_for_reset_confirmation'] = False
...
🗑️ **ریست کل اطلاعات** - فقط مدیران

💡 **نکته:** برای استفاده از دکمه‌های منو کلیک کنید.
    """
